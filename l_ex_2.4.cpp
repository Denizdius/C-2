...
float bisection_method(int n, float a[], float x1, float x2, float tol) {
    float xm, fm;
    while (fabs(x2 - x1) > tol) {
        xm = (x1 + x2) / 2;
        fm = polysum(n, a, xm);
        printf("f(%g)=%g, f(%g)=%g, f(%g)=%g\n", x1, polysum(n, a, x1), x2, polysum(n, a, x2), xm, fm);
        if (fm == 0.0) {
            printf("Bisection method: Root found at x = %g\n", xm);
            return xm;
        }
        if (fm * polysum(n, a, x1) < 0) {
            x2 = xm;
        } else {
            x1 = xm;
        }
...
int main() {
    ...
    float root_bisection = bisection_method(n, a, x1, x2, tol);
